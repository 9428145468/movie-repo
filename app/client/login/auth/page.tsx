...
const SignupForm = ({ setIsLogin }: { setIsLogin: (isLogin: boolean) => void; }) => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post('/users/register', { name, email, phoneNumber, password, role });
      console.log(response.data);
      router.push('/client/login.auth'); // Redirect to dashboard or login page after successful signup
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="bg-stone-950 p-8 rounded-xl shadow-lg">
      ...
      <form className="space-y-4" onSubmit={handleSignup}>
        ...
        <div>
          <button type="submit" className="w-full py-2 px-4 border border-transparent rounded-full shadow-sm text-sm font-medium text-stone-800 bg-stone-200 hover:bg-stone-300">
            Confirm
          </button>
        </div>
      </form>
